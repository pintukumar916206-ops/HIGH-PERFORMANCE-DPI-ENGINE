...
void DpiPipeline::addBlockApp(const std::string &name)
{
    ...
    for (auto &e : table)
    {
        if (name == e.key)
        {
            rules_.addBlockApp(e.val);
            return;
        }
    }
}

void DpiPipeline::feedPacket(const RawPacket &pkt)
{
    if (worker_queues_.empty())
        return;

    FiveTuple tuple;
    if (!quickTuple(pkt, tuple))
        return;

    int queue_idx = (tuple.src_ip + tuple.dst_ip + tuple.src_port + tuple.dst_port) % worker_queues_.size();
    worker_queues_[queue_idx]->try_enqueue(pkt);
    stats_.packets_read++;
    stats_.bytes_read += pkt.len;
}

void DpiPipeline::runLive(const std::string &interface)
{
    rules_.buildAutomata();
    live_mode_ = true;
    should_stop_ = false;

    std::cout << "Starting live capture on " << interface << " (" << cfg_.num_workers << " workers)\n";
    if (rules_.hasRules())
    {
    ...
    LiveCapture::Config cap_cfg;
    cap_cfg.interface = interface;
    cap_cfg.packet_count = -1;

    LiveCapture capturer;
    if (!capturer.init(cap_cfg))
    {
        std::cerr << "Failed to initialize capture on " << interface << "\n";
        return;
    }
    ...
    for (int i = 0; i < cfg_.num_workers; ++i)
        workers.push_back(std::make_unique<compat::thread>([this, i]
                                                           { workerThread(i); }));

    compat::thread writer([this]
                          { writerThread(); });
    std::unique_ptr<compat::thread> metrics;
    if (cfg_.live_stats)
        metrics = std::make_unique<compat::thread>([this]
                                                   {
            while (!should_stop_) {
    ...
    auto handle_pkt = [this](const Packet &pkt)
    {
        if (should_stop_)
            return;
        ...
        raw.timestamp.tv_sec = pkt.timestamp_us / 1000000;
        raw.timestamp.tv_usec = (pkt.timestamp_us % 1000000);

        feedPacket(raw);
    };

    capturer.start_capture(handle_pkt);
    should_stop_ = true;
...
void DpiPipeline::run()
{
    ...
    if (cfg_.live_stats)
        stats = std::make_unique<compat::thread>([this]
                                                 { statsThread(); });
...
void DpiPipeline::readerThread()
{
    ...
    {
        LOG_ERROR("could not open input file: " + cfg_.input_file);
        for (auto &q : worker_queues_)
            q->shutdown();
        return;
    }
...
void DpiPipeline::workerThread(int id)
{
    ...
    while (true)
    {
        ...
        if (pkt.ip_proto == proto::TCP)
            stats_.tcp_packets.fetch_add(1);
        else if (pkt.ip_proto == proto::UDP)
            stats_.udp_packets.fetch_add(1);
        else if (pkt.ip_proto == proto::ICMP)
...
void DpiPipeline::writerThread()
{
    ...
    {
        LOG_ERROR("could not open output file: " + cfg_.output_file);
        return;
    }
...
bool DpiPipeline::quickTuple(const RawPacket &raw, FiveTuple &out)
{
    ...
    if (etype == proto::ETH_IPV4)
    {
        ...
        uint32_t sip = (uint32_t(d[26]) << 24) | (uint32_t(d[27]) << 16) | (uint32_t(d[28]) << 8) | uint32_t(d[29]);
        uint32_t dip = (uint32_t(d[30]) << 24) | (uint32_t(d[31]) << 16) | (uint32_t(d[32]) << 8) | uint32_t(d[33]);
        out.setIPv4(sip, dip);
        out.protocol = d[23];
    ...
    if (!by_domain.empty())
    {
        ...
        for (size_t i = 0; i < show; ++i)
        {
            double share = total_dpkts > 0
                               ? 100.0 * double(ranked[i].second) / double(total_dpkts)
                               : 0.0;
...
void DpiPipeline::exportMetrics(const std::string &path) const
{
    uint64_t total = stats_.packets_read;
    uint64_t blocked = stats_.packets_blocked;
    double rate = blocked > 0 && total > 0 ? (100.0 * double(blocked) / double(total)) : 0.0;

    std::ofstream out(path);
    out << "{\n"
        << "  \"packets_total\": " << total << ",\n"
        << "  \"packets_blocked\": " << blocked << ",\n"
        << "  \"bytes_total\": " << stats_.bytes_read << ",\n"
        << "  \"throughput_pps\": " << std::fixed << std::setprecision(0) << stats_.throughputPps() << ",\n"
        << "  \"throughput_mbps\": " << std::setprecision(1) << stats_.throughputMBps() << ",\n"
        << "  \"block_rate\": " << std::setprecision(2) << rate << "\n"
        << "}\n";
    out.close();
}