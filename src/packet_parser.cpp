...
static size_t ip6_decode(const uint8_t* data, size_t len, size_t off, ParsedPacket& out) {
     ...
     * Format: Version|TrafficClass|FlowLabel|PayloadLen|NextHeader|HopLimit|Src|Dst
     */
    ...
    /* Parse Extension Headers (Hop-by-Hop, Routing, Fragment, etc.) */
    while (off < len) {
        /* Stop if we reach a Transport Layer or ICMP header */
        if (next == proto::TCP || next == proto::UDP || next == proto::ICMP) break;
        
        /* Each extension header is at least 8 bytes long */
        if (off + 8 > len) break;
        
        uint8_t ext_len = data[off + 1];
        size_t  ext = (static_cast<size_t>(ext_len) + 1) * 8;
        
        /* Defensive check against zero-length or malformed extensions */
        if (ext == 0 || off + ext > len) break;
