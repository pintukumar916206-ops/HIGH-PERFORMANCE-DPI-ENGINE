...
compat::optional<std::string> SNIExtractor::extract(const uint8_t* p,
    ...
    /* Parse Extensions (where SNI is located) */
    if (offset + 2 > len) return compat::nullopt;
    ...
    while (offset + 4 <= ext_end) {
        ...
        if (ext_type == 0x0000) {
            ...
            /* Ensure string data is within packet bounds before creating string */
            if (offset + 5 + sni_name_len > len) return compat::nullopt;
